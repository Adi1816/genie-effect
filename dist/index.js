...
function resolveMotion(source, target, direction) {
    ...
    const resolvedDirection = direction === "auto" ? (targetCenterY < sourceCenterY ? "up" : "down") : direction;
    return {
...
function screenY(axisY, direction) {
    return direction === "down" ? axisY : -axisY;
}
function getCurveRow(motion, progress, rowProgress) {
    ...
    const translateProgress = easeInOutQuad(clampProgress((progress - TRANSLATE_START_FRACTION) / (1 - TRANSLATE_START_FRACTION)));
    ...
    const curveProgress = axisDistance === 0
        ? 1
...
function getGenieFrame(motion, progress) {
    const rows = GENIE_CURVE_ROWS.map((rowProgress) => getCurveRow(motion, progress, rowProgress));
...
function prepareClone(source, initialRect) {
...
function createLayer(source, motion, options) {
    const layer = document.createElement("div");
    const panel = document.createElement("div");
    const clone = prepareClone(source, motion.initialRect);
    const parent = options.layerParent ??
        source.closest("[data-genie-root]") ??
        document.body;
...
export function runGenieEffect(source, target, options = {}) {
